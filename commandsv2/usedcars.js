...
module.exports = {
  ...
  async execute(interaction) {
    ...
    let descriptions =["Runs and drives", "Perfect condition", "It was my moms car", "I know what I have"]
    ...
    .addFields(
        {
            ...
            value:`${emotes.emotes.speed} ???\n${emotes.emotes.zero2sixty} ${car1.Acceleration}s\n${emotes.emotes.handling} ${car1.Handling}\n${emotes.emotes.weight} ${car1.Weight}\n\n${toCurrency(car1.Price)}\n${description1}\n${description2}`
        }
    )
    .setDescription("Used cars have a random price, and a random speed, take a gamble! The used car dealership resets weekly and each NPC has different cars that they put up for sale. So you can be sure all the NPCs will have a different exclusive car!\n**You can only buy 1 per day**")
  ...
  let filter2 = (btnInt) => {
    ...
    await i.update({embeds: [embed], fetchReply: true, components: [row]})
        }
    ...
    await i.update({embeds: [embed], fetchReply: true, components: [row]})
          ...
          if (cooldown !== null && timeout - (Date.now() - cooldown) > 0) {
            ...
            await i.update({ content: `You've already bought a used car today\n\nBuy another in ${time}.`, fetchReply: true });
            return;
          }
          ...
          if(cash < tobuy.Price) return i.update("You don't have enough cash for this car!")
