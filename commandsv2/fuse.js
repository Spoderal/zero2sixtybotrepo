...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (parttoinstall == "txexhaust") {
      ...
      if (nuke.length == 0)
        return await interaction.reply(`You're missing nuclear core!`);
      if (hook.length == 0)
        return await interaction.reply(`You're missing car hook!`);
      if (pistons.length == 0)
      ...
      if (userdata.rocket == true)
        return interaction.reply("You've already made an epic rocket engine!");
