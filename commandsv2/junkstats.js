...
module.exports = {
  ...
  async execute(interaction) {
    ...
    let list = cars.Cars
    car = car.toLowerCase()
    if(!usercars.includes(`${car.toLowerCase()}`)) return interaction.reply("You dont have that car!")
    let exhaust = db.fetch(`${cars.Cars[car].Name}exhaust_${uid}`) || 'Cracked Exhaust'
    ...
    let suspension = db.fetch(`${cars.Cars[car].Name}suspension_${uid}`) || 'Broken Suspension'
    let speed = db.fetch(`${cars.Cars[car].Name}speed_${uid}`) || 0
    let restoration = db.fetch(`${cars.Cars[car].Name}restoration_${uid}`)
    let gearbox = db.fetch(`${cars.Cars[car].Name}gearbox_${uid}`) || 'No Gearbox'
    
    let body = db.fetch(`${cars.Cars[car].Name}body_${uid}`) || 'Rusted'
    let carimage = db.fetch(`${cars.Cars[car].Name}livery_${uid}`) || cars.Cars[car].Image
    ...
    let licensePlate = db.fetch(`${cars.Cars[car].Name}license_plate_${uid}`) || '000000'
    var resale = db.fetch(`${cars.Cars[car].Name}resale_${uid}`) || 'Not sellable'
    
    let speedemote = "<:speeddefault:932423319020531753>"
    let caremote = cars.Cars[car].Emote || "<:z_none:898352936785178645>"
    
    console.log(engine)
    let engineemote = parts.Parts[engine.toLowerCase()].Emote || "<:enginedefault:932419391587483688>"
    let gearboxemote = parts.Parts[gearbox.toLowerCase()].Emote || "<:enginedefault:932419391587483688>"
    
    let exhaustemote = parts.Parts[exhaust.toLowerCase()].Emote || "<:exhaustdefault:932419391650418709>"
    let intakeemote = parts.Parts[intake.toLowerCase()].Emote || "<:intakedefault:932419391734292561>"
    let tiresemote = parts.Parts[tires.toLowerCase()].Emote || "<:tiresdefault:932419392036306954>"
    let suspensionemote = parts.Parts[suspension.toLowerCase()].Emote || "<:suspensiondefault:932419391847563314>"
    let clutchemote = parts.Parts[clutch.toLowerCase()].Emote || "<:defaultclutch:935084786748370954>"
    ...
    let color = "DANGER"
    if(engine !== "No Engine" && exhaust !== "Cracked Exhaust" && intake !== "No Intake" && tires !== "Flat Tires" && gearbox !== "No Gearbox" && suspension !== "Broken Suspension" && body !== "Rusted" && clutch !== "No Clutch"){
      color = "SUCCESS"
    }
    row.addComponents(
      new MessageButton()
  ...
  collector.on('collect', async (i, user) => {
    ...
    if(i.customId.includes("exhaust")){
      ...
      if(!items.includes("wrench")){
        if(!userparts.includes(restorepart.toLowerCase())) return i.update({content:"You don't own that part!"})

      }

  

      db.set(`${cars.Cars[car.toLowerCase()].Name}exhaust_${interaction.user.id}`, `${parts.Parts[restorepart.toLowerCase()].Name}`)
      let engine = db.fetch(`${cars.Cars[car].Name}engine_${uid}`) || 'No Engine'
      let exhaust = db.fetch(`${cars.Cars[car].Name}exhaust_${uid}`) || 'Cracked Exhaust'
      let tires = db.fetch(`${cars.Cars[car].Name}tires_${uid}`) || 'Flat Tires'
      let intake = db.fetch(`${cars.Cars[car].Name}intake_${uid}`) || 'No Intake'
      ...
      let nexhaustemote = parts.Parts[nexhaust.toLowerCase()].Emote || "<:exhaustdefault:932419391650418709>"
      let row = new MessageActionRow()
    ...
    row.addComponents(
      ...
      let suspensionemote = parts.Parts[suspension.toLowerCase()].Emote || "<:suspensiondefault:932419391847563314>"
      let clutchemote = parts.Parts[clutch.toLowerCase()].Emote || "<:defaultclutch:935084786748370954>"
      embed.addField(`${engineemote} Engine`, `${engine}`, true)
      ...
      db.set(`${cars.Cars[car.toLowerCase()].Name}engine_${interaction.user.id}`, `${parts.Parts[restorepart.toLowerCase()].Name}`)
      let exhaust = db.fetch(`${cars.Cars[car].Name}exhaust_${uid}`) || 'Cracked Exhaust'
      let tires = db.fetch(`${cars.Cars[car].Name}tires_${uid}`) || 'Flat Tires'
      let intake = db.fetch(`${cars.Cars[car].Name}intake_${uid}`) || 'No Intake'
      let clutch = db.fetch(`${cars.Cars[car].Name}clutch_${uid}`) || 'No Clutch'
      ...
      db.set(`${cars.Cars[car.toLowerCase()].Name}clutch_${interaction.user.id}`, `${parts.Parts[restorepart.toLowerCase()].Name}`)
      let exhaust = db.fetch(`${cars.Cars[car].Name}exhaust_${uid}`) || 'Cracked Exhaust'
    let tires = db.fetch(`${cars.Cars[car].Name}tires_${uid}`) || 'Flat Tires'
    let intake = db.fetch(`${cars.Cars[car].Name}intake_${uid}`) || 'No Intake'
    let clutch = db.fetch(`${cars.Cars[car].Name}clutch_${uid}`) || 'No Clutch'
    ...
    let suspensionemote = parts.Parts[suspension.toLowerCase()].Emote || "<:suspensiondefault:932419391847563314>"
      ...
      db.set(`${cars.Cars[car.toLowerCase()].Name}suspension_${interaction.user.id}`, `${parts.Parts[restorepart.toLowerCase()].Name}`)
      let exhaust = db.fetch(`${cars.Cars[car].Name}exhaust_${uid}`) || 'Cracked Exhaust'
    let tires = db.fetch(`${cars.Cars[car].Name}tires_${uid}`) || 'Flat Tires'
    let intake = db.fetch(`${cars.Cars[car].Name}intake_${uid}`) || 'No Intake'
    let clutch = db.fetch(`${cars.Cars[car].Name}clutch_${uid}`) || 'No Clutch'
    ...
    let suspensionemote = parts.Parts[suspension.toLowerCase()].Emote || "<:suspensiondefault:932419391847563314>"
      ...
      db.set(`${cars.Cars[car.toLowerCase()].Name}intake_${interaction.user.id}`, `${parts.Parts[restorepart.toLowerCase()].Name}`)
      let exhaust = db.fetch(`${cars.Cars[car].Name}exhaust_${uid}`) || 'Cracked Exhaust'
    let tires = db.fetch(`${cars.Cars[car].Name}tires_${uid}`) || 'Flat Tires'
    let intake = db.fetch(`${cars.Cars[car].Name}intake_${uid}`) || 'No Intake'
    let clutch = db.fetch(`${cars.Cars[car].Name}clutch_${uid}`) || 'No Clutch'
    ...
    row.addComponents(
      ...
      db.set(`${cars.Cars[car.toLowerCase()].Name}tires_${interaction.user.id}`, `${parts.Parts[restorepart.toLowerCase()].Name}`)
      let exhaust = db.fetch(`${cars.Cars[car].Name}exhaust_${uid}`) || 'Cracked Exhaust'
    let tires = db.fetch(`${cars.Cars[car].Name}tires_${uid}`) || 'Flat Tires'
    let intake = db.fetch(`${cars.Cars[car].Name}intake_${uid}`) || 'No Intake'
    let clutch = db.fetch(`${cars.Cars[car].Name}clutch_${uid}`) || 'No Clutch'
    ...
    let suspensionemote = parts.Parts[suspension.toLowerCase()].Emote || "<:suspensiondefault:932419391847563314>"
      ...
      db.set(`${cars.Cars[car.toLowerCase()].Name}gearbox_${interaction.user.id}`, `${parts.Parts[restorepart.toLowerCase()].Name}`)
      let exhaust = db.fetch(`${cars.Cars[car].Name}exhaust_${uid}`) || 'Cracked Exhaust'
    let tires = db.fetch(`${cars.Cars[car].Name}tires_${uid}`) || 'Flat Tires'
    let intake = db.fetch(`${cars.Cars[car].Name}intake_${uid}`) || 'No Intake'
    let clutch = db.fetch(`${cars.Cars[car].Name}clutch_${uid}`) || 'No Clutch'
    ...
    let suspensionemote = parts.Parts[suspension.toLowerCase()].Emote || "<:suspensiondefault:932419391847563314>"
      ...
      db.set(`${cars.Cars[car.toLowerCase()].Name}body_${interaction.user.id}`, `${parts.Parts[restorepart.toLowerCase()].Name}`)
      let exhaust = db.fetch(`${cars.Cars[car].Name}exhaust_${uid}`) || 'Cracked Exhaust'
    let tires = db.fetch(`${cars.Cars[car].Name}tires_${uid}`) || 'Flat Tires'
    let intake = db.fetch(`${cars.Cars[car].Name}intake_${uid}`) || 'No Intake'
    let clutch = db.fetch(`${cars.Cars[car].Name}clutch_${uid}`) || 'No Clutch'
    ...
    let suspensionemote = parts.Parts[suspension.toLowerCase()].Emote || "<:suspensiondefault:932419391847563314>"
      ...
      if(engine !== "No Engine" && exhaust !== "Cracked Exhaust" && intake !== "No Intake" && tires !== "Flat Tires" && gearbox !== "No Gearbox" && suspension !== "Broken Suspension" && body !== "Rusted" && clutch !== "No Clutch"){
        ...
        if(db.fetch(`carsrrestored_${uid}`) >= 5 && !achievements.includes("rusty racer")){
            interaction.channel.send(`You just completed the achievement "Rusty Racer" and earned a rust helmet!`)
        ...
        db.set(`${cars.Cars[fullrestored.Name.toLowerCase()].Name}speed_${uid}`, cars.Cars[fullrestored.Name.toLowerCase()].Speed)
        db.set(`${cars.Cars[fullrestored.Name.toLowerCase()].Name}060_${uid}`, cars.Cars[fullrestored.Name.toLowerCase()]["0-60"])
        db.set(`${cars.Cars[fullrestored.Name.toLowerCase()].Name}resale_${uid}`, cars.Cars[fullrestored.Name.toLowerCase()].sellprice)
        
        db.push(`cars_${uid}`, `${fullrestored.Name.toLowerCase()}`)

        let newusercars = db.fetch(`cars_${uid}`)
        for (var i = 0; i < 1; i ++) newusercars.splice(newusercars.indexOf(car.toLowerCase()), 1)
    ...
    for (var i = 0; i < 1; i ++) userparts.splice(userparts.indexOf(restorepart.toLowerCase()), 1)
