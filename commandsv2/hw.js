  ...
  async execute(interaction) {
    ...
    collector.on("end", async () => {
      ...
      let embed = new discord.MessageEmbed()
        .setTitle("3...2...1....GO!")
        .addField(
          `${user.username}'s ${cars.Cars[user1car.toLowerCase()].Emote} ${
            cars.Cars[user1car.toLowerCase()].Name
          }`,
          `Speed: ${user1carspeed}\n\n0-60: ${newzero1}s`
        )
        .addField(
          `${user2.username}'s ${cars.Cars[user2car.toLowerCase()].Emote} ${
      ...
      let x = setInterval(() => {
        ...
        if (timer >= 30) {
          if (tracklength > tracklength2) {
            ...
            if (
              dailytask1 &&
              dailytask1.task == "Win 1 pvp race" &&
              !dailytask1.completed
            ) {
              db.set(`dailytask_${user.id}.completed`, true);
              db.add(`cash_${user.id}`, dailytask1.reward);
              msg2.reply(
                `${user}, you just completed your daily task "${dailytask1.task}"!`
              );
            }
            if (
              weekytask1 &&
              weekytask1.task == "Win a pvp race in a 1994 Porsche 911" &&
              !weekytask1.completed &&
              user1car.toLowerCase() == "1994 porsche 911"
            ) {
              db.set(`weeklytask_${user.id}.completed`, true);
              db.add(`cash_${user.id}`, weekytask1.reward);
              msg2.reply(
                `${user}, you just completed your weekly task "${weekytask1.task}"!`
              );
            }
            ...
            if (
              dailytask2 &&
              dailytask2.task == "Win 1 pvp race" &&
              !dailytask2.completed
            ) {
              db.set(`dailytask_${user2.id}.completed`, true);
              db.add(`cash_${user2.id}`, dailytask2.reward);
              msg2.reply(
                `${user2}, you just completed your daily task "${dailytask2.task}"!`
              );
            }
            if (
              weekytask2 &&
              weekytask2.task == "Win a pvp race in a 1994 Porsche 911" &&
              !weekytask2.completed &&
              user2car.toLowerCase() == "1994 porsche 911"
            ) {
              db.set(`weeklytask_${user2.id}.completed`, true);
              db.add(`cash_${user2.id}`, weekytask2.reward);
              msg2.reply(
                `${user2}, you just completed your weekly task "${weekytask2.task}"!`
              );
            }
            if (db.fetch(`racexp_${user2.id}`) >= newrankrequired2) {
              if (db.fetch(`racerank_${user2.id}`) < 20) {
                ...
                msg2.reply(
                  `${user2}, you just ranked up your race skill to ${db.fetch(
                    `racerank_${user.id}`
                  )}!`
                );
              }
