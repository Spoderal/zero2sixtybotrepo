...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (ubusiness && ubusiness.CustomerD < ubusiness.Customers && ubusiness.Cooldown !== null && timeout - (Date.now() - ubusiness.Cooldown) < 0) {
    ...
    if(userdata.business.Name){
        businessrow = new ActionRowBuilder()
        .setComponents(
            new ButtonBuilder()
...
console.log(diff)
    ...
    let msg = await interaction.reply({embeds: [embed], components: [businessrow], fetchReply: true})
    ...
    let businessbuttons = new ActionRowBuilder()
    .setComponents(
        new ButtonBuilder()
        .setCustomId("carwash")
        .setLabel("Car Wash")
    ...
    collector.on('collect', async (i) => {
        if(i.customId == "create"){
           ...
           await i.update({embeds: [embed], components: [businessbuttons]})
            ...
            let business = businessarr.filter((business) => business.Name == userdata.business.Business)
            ...
            for(let u in upgrades){
                ...
                if(userdata.business.Level >= upg.Level){
                    if(userdata.business.Upgrades && !userdata.business.Upgrades.includes(upg)){
                        
                        upgraded.push(`${upg.Emote} **${upg.Name}** ${upg.Description} : ${toCurrency(upg.Cost)}`)
            ...
            embed1.setTitle("Business Upgrades")
            ...
            const collector2 = msg.createMessageComponentCollector({filter: filter, time: 30000})
            
         
            await i.update(em)

            collector2.on('collect', async (i) => {
                let upgrade = business[0].Upgrades.filter((upgrade) => upgrade.ID == i.customId)
            ...
            let editrow = new ActionRowBuilder()
            .setComponents(
                new ButtonBuilder()
                .setCustomId("name")
                .setLabel("Edit Name")
            ...
            i.update({content: `Send the name you want to set`, fetchReply: true})

            let filter = (msg) => {
                return interaction.user.id === msg.author.id;
              };
            let collector3 = interaction.channel.createMessageCollector({filter: filter })
            ...
            embed.setTitle(`Created your business! View it by sending the command again.`)
