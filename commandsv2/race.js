...
module.exports = {
  ...
  async execute(interaction) {
    const dorace = (speed, speed2, handling, handling2, weight, weight2, acceleration, acceleration2) => {
    ...
    if(selected.Gas <= 0) return interaction.reply(`You're out of gas! Use \`/gas\` to fill up for the price of gas today! Check the daily price of gas with \`/bot\``)
    let car2
    let raceoption = interaction.options.getString("race")
    let tieroption = interaction.options.getNumber("tier")
    let cartofilter = []
    if(tieroption == 1){
       cartofilter = carsarray.filter((car) => car.Speed <= 300  && car.Class == "D")
    }
   else if(tieroption == 2){
       cartofilter = carsarray.filter((car) => car.Speed <= 400 && car.Class == "C")
    }
    else if(tieroption == 3){
       cartofilter = carsarray.filter((car) => car.Speed <= 500  && car.Class == "B")
    }
    else if(tieroption == 4){
       cartofilter = carsarray.filter((car) => car.Speed <= 600  && car.Class == "A")
    }
    else if(tieroption == 5){
       cartofilter = carsarray.filter((car) => car.Speed <= 700  && car.Class == "A" || car.Speed <= 700 && car.Class == "S")
    }
    else if(tieroption == 6){
      cartofilter = carsarray.filter((car) => car.Speed <= 800  && car.Class == "A" || car.Speed <= 800 && car.Class == "S")
   }
   else if(tieroption == 7){
    cartofilter = carsarray.filter((car) => car.Speed <= 900  && car.Class == "A" || car.Speed <= 900 && car.Class == "S")
 }
 else if(tieroption == 8){
  cartofilter = carsarray.filter((car) => car.Speed >= 1000  && car.Class == "A" || car.Speed >= 1000 && car.Class == "S")
}
    ...
    let speedscore2 = speed2 * 10
      ...
      dorace(speedscore, speedscore2, handling, handling2, weightscore, weightscore2, acceleration, acceleration2)
    ...
    let speedscore2 = speed2 * 15
      ...
      dorace(speedscore, speedscore2, handlingscore, handlingscore2, weightscore, weightscore2, acceleration, acceleration2)
    ...
    .setColor(colors.blue)
    .addFields(
      {name: `${helmetdb.Pfps[userpfp.toLowerCase()].Emote} Your ${selected.Emote} ${selected.Name}`, value: `${emotes.speed} HP: ${selected.Speed}\n${emotes.zero2sixty} Acceleration: ${selected.Acceleration}s\n${emotes.handling} Handling: ${selected.Handling}\n${emotes.weight} Weight: ${selected.WeightStat}`, inline: true},
      {name: `${car2.Emote} ${car2.Name}`, value: `${emotes.speed} HP: ${car2.Speed}\n${emotes.zero2sixty} Acceleration: ${car2[`0-60`]}s\n${emotes.handling} Handling: ${car2.Handling}\n${emotes.weight} Weight: ${car2.Weight}`, inline: true}
    ...
    setTimeout(async () => {
      if(winner =="Player"){
        ...
        else if(raceoption == "trackrace" && possiblekey > 10 && tieroption <= 4){
          let randomamount = randomRange(1, 3)
          rewards.push(`${emotes.rareKey} ${randomamount}`)
          userdata.rkeys += randomamount
        }
        else if(raceoption == "trackrace" && possiblekey > 10 && tieroption >= 5){
          let randomamount = randomRange(1, 3)
