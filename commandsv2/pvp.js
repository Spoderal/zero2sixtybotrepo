...
const { ActionRowBuilder, ButtonBuilder, AttachmentBuilder, EmbedBuilder } = require("discord.js");
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    subcommand
    .setName("rank")
    .setDescription("View your PVP rank")
  ...
  async execute(interaction) {
    ...
    let userpvprank = userdata.pvprank || {Rank: "Silver", Wins: 0, Losses: 0, Rewards: 0}
    let userpvprank2 = userdata2.pvprank || {Rank: "Silver", Wins: 0, Losses: 0, Rewards: 0}
    let urankslist = []
...
if(!selected.WeightStat){
  ...
  attachment = new AttachmentBuilder(await canvas.toBuffer(), { name: 'profile-image.png' });
          ...
          userdata.update()
            ...
            let rewardinreward = userrank[0].rewards.filter(reward => reward.number == rewardtogive) || 0
    ...
    if(rewardinreward.length !== 0){
      ...
      if (rewardinreward[0].reward.endsWith("Cash")) {
      ...
      else if (rewardinreward[0].reward.endsWith("Legendary Barn Map")) {
        let amount = Number(rewardinreward[0].reward.split(" ")[0]);
        userdata.lmaps += amount;
        userdata.pvprank.Rewards += 1;
        earnings.push(`${emotes.barnMapLegendary} +${amount}`)
      } 
      else if (rewardinreward[0].reward.endsWith("Super Wheelspins")) {
        let amount = Number(rewardinreward[0].reward.split(" ")[0]);
        ...
        earnings.push(`${emotes.gold} +${amount}`)
          ...
          if(earnings.length > 0){
            ...
            userdata.update()
            console.log(nextuser1rank[0].wins)
            if(userdata.pvprank.Wins >= nextuser1rank[0].wins){
            ...
            if(userdata2.pvprank.Wins >= nextuser2rank[0].wins){
              userdata2.pvprank.Wins = 0
              userdata2.pvprank.Losses = 0
              userdata2.pvprank.Rank = `${nextuser2rank[0].name}`
            }
  ...
  attachment = new AttachmentBuilder(await canvas.toBuffer(), { name: 'profile-image.png' });
          ...
          userdata.update()
            ...
            let rewardinreward = userrank2[0].rewards.filter(reward => reward.number == rewardtogive) || 0
            console.log(rewardinreward)
    if(rewardinreward.length !== 0){
    ...
    let newwins = nextrank[0].wins || 0

    console.log(nextrank)

    let embed = new EmbedBuilder()
    .setAuthor({name: `${user.username} - PVP Rank`, iconURL:`${helmetdb.Pfps[userdata.helmet.toLowerCase()].Image}`})
    .setDescription(`${pvpranks[pvprank.Rank.toLowerCase()].emote} ${pvprank.Rank}\n\nWins: ${pvprank.Wins}\n\nWins Needed: ${newwins}`)
