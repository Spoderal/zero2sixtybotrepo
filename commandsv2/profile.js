      ...
      collector.on("collect", async (i) => {
        if(i.customId == "helmets"){
           ...
           for(let h in helmlist[page]){
            ...
            displayhelms.push(`${profilepics[helm.toLowerCase()].Emote} ${profilepics[helm.toLowerCase()].Name}`)
           }
           ...
           i.update({embeds: [embed], components: [row, row2], fetchReply: true})
        }
          ...
          if(page > items.length) return i.update({content: "You don't have anymore pages!"})
