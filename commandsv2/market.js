        ...
        if(subcommand == "list"){
            ...
            if(cardb.Cars[item.toLowerCase()]){
              let filteredcar = userdata.cars.filter(
                ...
                if(sellprice > price) return interaction.reply(`Your price must at least be ${toCurrency(sellprice)}, this is to prevent things getting undervalued.`)
                 obj = {
          ...
          else return interaction.reply(`Error! The item you're trying to list either doesn't exist or is not marketable.`)
          ...
          if(interaction.options.getString("id")){
            ...
            let suspensionemote = partdb.Parts[suspension.toLowerCase()]?.Emote || "🔵";
            ...
            let intercooleremote = partdb.Parts[intercooler.toLowerCase()]?.Emote || "🔵";
            ...
            for(let m in market){
                ...
                if(cardb.Cars[listing.item]){
                    marketdisplay.push(`\`ID ${listing.id}\`: ${cardb.Cars[listing.item].Emote} ${cardb.Cars[listing.item].Name} - ${toCurrency(listing.price)}`)

                }
                else if(partdb.Parts[listing.item.toLowerCase()]) {
                    marketdisplay.push(`\`ID ${listing.id}\`: ${partdb.Parts[listing.item.toLowerCase()].Emote} ${partdb.Parts[listing.item.toLowerCase()].Name} - ${toCurrency(listing.price)} **x${listing.amount}**`)
                }
                else if(itemdb.Other[listing.item.toLowerCase()]) {
                  marketdisplay.push(`\`ID ${listing.id}\`: ${itemdb.Other[listing.item.toLowerCase()].Emote} ${itemdb.Other[listing.item.toLowerCase()].Name} - ${toCurrency(listing.price)} **x${listing.amount}**`)
              }
              else if(currencies[listing.item.toLowerCase()]){
                marketdisplay.push(`\`ID ${listing.id}\`: ${currencies[listing.item.toLowerCase()].Emote} ${currencies[listing.item.toLowerCase()].Name} - ${toCurrency(listing.price)} **x${listing.amount}**`)
              }
            }
      ...
      if (current !== page) {
        ...
        return i.update({ content: "No pages left!" });
          ...
          if(!itemtobuy[0]) return interaction.reply('Thats not an item on the market!')
           ...
           if(parseInt(itemtobuy.price) > cash) return interaction.reply("You can't afford this!")
          ...
          userdata.save()
          try {
            let userfromitem = await interaction.client.users.fetch(itemtobuy.user.id);
