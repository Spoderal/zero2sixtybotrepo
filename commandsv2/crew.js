...
module.exports = {
  data: new SlashCommandBuilder()
   ...
   ,
    ...
    let mlength = crew2.members.length
       ...
       .setThumbnail(icon)
       .addField("Information", `${mlength} members\n\nRank ${crew2.Rank}\n\nRP: ${total}\n\nCrew Leader: ${crew2.owner.username}#${crew2.owner.discriminator}`, true)
      ...
      const collector = emb.createMessageComponentCollector({
        ...
        collector.on('collect', async (i) => {
          if(i.customId.includes("season")){
            ...
            embed.fields = []
            embed.setTitle(`Info for ${crew2.name}`)
            .setThumbnail(icon)
            .addField("Information", `${crew2.members.length} members\n\nRank ${crew2.Rank}\n\nRP: ${total}\n\nCrew Leader: ${crew2.owner.username}#${crew2.owner.discriminator}`, true)
            .addField("Leaderboard", `${finalLb}`, true)
        ...
        let crew2 = global.crews.filter(crew => crew.name == crewname)
        if(crew2.length == 0) return interaction.reply("That crew doesn't exist!")
        ...
        let crew = userdata.crew
        if(crew) return interaction.reply("You're already in a crew! If you're a member, leave with /crew leave, and if you're the owner, delete it with /crew delete")
      ...
      let crew2 = crews.filter(crew => crew.name == crewname)
      if(!crew2[0]) return interaction.reply("That crew doesn't exist!")
      if(crew2.ownerid !== interaction.user.id) return interaction.reply("You're not the crew owner!")
  ...
  let crew2 = crews.filter(crew => crew.name == crewname)
  if(!crew2[0]) return interaction.reply("That crew doesn't exist!")
  if(crew2[0].owner.id !== uid) return interaction.reply("You're not the crew owner!")
  
  let crewlist = global.crews
  
  
  interaction.reply("Are you sure? This will permanently remove all perks from all members. Say yes to confirm, and anything else to cancel.")
  const filter = (m = Discord.Message) => {
    return m.author.id === interaction.user.id
  }
  let collector = interaction.channel.createMessageCollector({
  ...
  collector.on('collect', m => {
    if(m.content.toLowerCase() == "yes"){
      let crewobj = crew2[0]
      for (var i = 0; i < 1; i ++) crewlist.splice(crewlist.indexOf(crewobj), 1)
