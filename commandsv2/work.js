...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if(option == "joblist"){
            ...
            .setTitle("Job List")
            .addField("Mechanic", `<:helmet_mechanic:967208601145974875>\nPrestige Required: 2\nStarting Rank: Tire Installer\nStarting Salary: $150/6 hours`, true)
            .addField("Police", `<:helmet_police:967209460630171658>\nPrestige Required: 3\nStarting Rank: Cadet\nStarting Salary: $200 per chase\n\nChase other players, or chase bots in wanted!`, true)
            .addField("YouTuber", `<:helmet_youtube:967208601435398164>\nPrestige Required: 4\nStarting Rank: 100 Subscribers\nStarting Salary: $50/1 hour`, true)
            .addField("Zero2Sixty Programmer", `<:helmet_programmer:973390981179260928>\nPrestige Required: 7\nStarting Rank: Jr Web Developer\nStarting Salary: $400/6 hours`, true)
            .addField("Pizza Delivery", `<:pizzaman:978186078802559007>\nPrestige Required: 5\nStarting Rank: Pizza Boy\nStarting Salary: $250/3 hours`, true)
            ...
            if(actjob == "mechanic"){
                ...
                let embed = new Discord.MessageEmbed()
                .setTitle(`Working...`)
                .setDescription(`Duties: ${question.Question}\n\n__Options__:\n${question.Options.join('\n')}`)
                .setColor("#60b0f4")
                .setThumbnail(jobsdb.Jobs[actjob].Helmet)
                interaction.reply({embeds: [embed]})
                job.worked = Date.now()
                ...
                let choice, collector = interaction.channel.createMessageCollector({
                    filter,
                    max: 1,
                    time: 1000 * 20
                })
                ...
                let videotypes1 = {
                    ...
                    "car review":{
                        Cars:["1999 Honda Civic", "1991 Toyota MR2", "1999 Mitsubishi Eclipse"],
                        Trending: false,
                    },
                    "podcast":{
                        Topics:["JDM or American cars?", "My favorite brand", "Best car ever"],
                        Trending: false,
                    }
                }
                ...
                let type
                    ...
                    interaction.channel.send("Choose a video type!").then(emb => {
                        ...
                        let rcollector = emb.createReactionCollector({ filter, time: 300000 })
                        rcollector.on('collect', async (r, user) => {
                            emb.reactions.cache.get(r.emoji.name).users.remove(user.id)
                            emb.reactions.cache.get(r.emoji.name).users.remove(interaction.client.id)
                            ...
                            if(type == "gaming"){
                                ...
                                let gameimages = {
                                    ...
                                    "FortZa":{
                                        Image:"https://i.pcmag.com/imagery/reviews/06vWN3yxcpku86WEFg5CoXb-5..v1633546079.jpg"
                                    },
                                    "Gran Brrismo":{
                                        Image:"https://car-images.bauersecure.com/pagefiles/76993/gt-sport_review_12.jpg"
                                },
                                ...
                                interaction.channel.send({embeds: [embed]}).then(emb => {
                                    ...
                                    let filter = (_, u) => u.id === interaction.user.id
                                    let rcollector2 = emb.createReactionCollector({ filter, time: 300000 })
                    ...
                    userdata.save()
                                ...
                                interaction.channel.send(`Choose a rating to give to the ${cartoreview} out of 10, if your rating is close to what most people think you get a reward!`)
                                ...
                                collector.on('collect', msg => {
                                    ...
                                    if(isNaN(numbertorate)) return msg.reply("Thats not a number!")

                                    if(numbertorate > 10) return msg.reply("Thats greater than 10!")
                                    let possiblenumbers
                                    if(numbertorate == 1){
                ...
                if(randomgame == "clicker"){
                    ...
                    interaction.reply({content: `Click the ${randomcolor} button`, components: [row]})
                    ...
                    let typing = ["hello world", "zero2sixty typing", "slash commands", `console.log("hi")`]
                    let randomtype = lodash.sample(typing)
                  interaction.reply({content: `Type \`${randomtype}\` in 10 seconds!`})
                  ...
                  collector.on('end', collected => {
                      if(collected.size == 0) return interaction.channel.send("You took too long!")
                  }) 
                ...
                userdata.update()
                    ...
                    interaction.reply(`Remember that this customer wants ${randpizza} pizzas.`)
                    setTimeout(() => {
                    ...
                    interaction.editReply({content: `How many pizzas did they want?`, components: [row]})
