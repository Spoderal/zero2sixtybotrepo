...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if(interaction.user.id !== "890390158241853470" && interaction.user.id !== "474183542797107231" && interaction.user.id !== "152079857793105920" && interaction.user.id !== "699794627095429180" && interaction.user.id !== "937967206652837928" && interaction.user.id !== "670895157016657920")return interaction.reply({content:"You don't have permission to run this command.", ephemeral: true})
       ...
       if(tier == "spj"){
        ...
        db.set(`1989 Nissan Skyline R32speed_${user.id}`, cars.Cars["1989 nissan skyline r32"].Speed)
        db.set(`1989 Nissan Skyline R32handling_${user.id}`, cars.Cars["1989 nissan skyline r32"].Handling)
        db.set(`1989 Nissan Skyline R32060_${user.id}`, cars.Cars["1989 nissan skyline r32"]['0-60'])
    ...
    if(tier == "spa"){
        ...
        db.set(`2010 Ford Mustangspeed_${user.id}`, cars.Cars["2010 ford mustang"].Speed)
        db.set(`2010 Ford Mustanghandling_${user.id}`, cars.Cars["2010 ford mustang"].Handling)
        db.set(`2010 Ford Mustang060_${user.id}`, cars.Cars["2010 ford mustang"]['0-60'])
        db.add(`cash_${user.id}`, 25000)
    ...
    if(tier == "spe"){
        ...
        db.set(`2002 BMW M3 GTRspeed_${user.id}`, cars.Cars["2002 bmw m3 gtr"].Speed)
        db.set(`2002 BMW M3 GTRhandling_${user.id}`, cars.Cars["2002 bmw m3 gtr"].Handling)
        db.set(`2002 BMW M3 GTR060_${user.id}`, cars.Cars["2002 bmw m3 gtr"]['0-60'])
        db.add(`cash_${user.id}`, 25000)
