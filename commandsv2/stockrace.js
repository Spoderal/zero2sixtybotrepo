...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("stockrace")
    .setDescription("Race a bot with stock cars")
  ...
  async execute(interaction) {
    ...
    let timeout = 180000;
    if (
      cooldowndata.stockracing !== null &&
      timeout - (Date.now() - cooldowndata.stockracing) > 0
    ) {
      let time = ms(timeout - (Date.now() - cooldowndata.stockracing));
    ...
    cooldowndata.stockracing = Date.now();
    cooldowndata.save();
    ...
    let embed = new EmbedBuilder()
      .setTitle(`Racing Tier ${bot} Stock Race`)
    ...
    let i2 = setInterval(async () => {
      ...
      if (tracklength2 <= 0 && i2 !== null) {
        ...
        embed.setTitle(`Tier ${bot} Stock Race lost!`);
        ...
        earnings.push(`<:STOCKPOINTS:1077426546077347871> +${stockpoints}`);
