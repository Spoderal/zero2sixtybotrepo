...
module.exports = {
  ...
  async execute(interaction) {
   ...
   if(subcommandfetch == "car_part" && item && list[item.toLowerCase()]){
      ...
      .addField(`Acceleration`, `${accelerationemote} ${carindb['0-60']}`, true)
  ...
  let embed = new Discord.MessageEmbed()
  .setTitle(`Stats for ${interaction.user.username}'s ${carindb.Emote} ${carindb.Name}`)
  .addField(`Speed`, `${speedemote} ${carindb.Speed} MPH`, true)
  .addField(`Acceleration`, `${accelerationemote} ${carindb.Acceleration}s`, true)
  .addField(`Handling`, `${handlingemote} ${carindb.Handling}`, true)
...
collector2.on('collect', async (i, user) => {
  ...
  if(i.customId.includes("parts")){
    ...
    let suspensionemote = partindb[suspension.toLowerCase()].Emote || '🔵'
    ...
    let embed = new Discord.MessageEmbed()
  .setTitle(`Parts for ${interaction.user.username}'s ${carindb.Emote} ${carindb.Name}`)
  ...
  i.update({embeds: [embed]})
    ...
    .setTitle(`Stats for ${interaction.user.username}'s ${carindb.Emote} ${carindb.Name}`)
    .addField(`Speed`, `${speedemote} ${carindb.Speed}`, true)
    .addField(`Acceleration`, `${accelerationemote} ${carindb.Acceleration}`, true)
    .addField(`Handling`, `${handlingemote} ${carindb.Handling}`, true)
    .addField(`Drift`, `${handlingemote} ${cardrift}`, true)
    .addField(`Sell Price`, `$${numberWithCommas(sellprice)}`, true)
    .addField(`\u200b`, `\u200b`, true)
   ...
   else if(subcommandfetch == "car_part" && partdb.Parts[item.toLowerCase()]){
    ...
    if(partindb.AddHandling){
      stats.push(`<:handling:983963211403505724> Handling: +${partindb.AddHandling}`)
    }
    if(partindb.DecreaseHandling){
      stats.push(`<:handling:983963211403505724> Handling: -${partindb.DecreaseHandling}`)
    }
