...
,
    async execute(interaction) {
        ...
        if(subcommand == "create"){
            ...
            let newid = storeid + 1
            db.set(`usedcarshop_${user.id}`, {Name: name, Sales: 0, Listed: 0, Profit: 0, Owner: user.tag, ID: newid, Current: []})
            db.add(`numused`, 1)
            ...
            if(!usedshop) return interaction.reply(`You don't have a dealership open!`)

            let carname = interaction.options.getString("name")
            let carprice = interaction.options.getNumber("price")

            if(!cardb.Cars[carname.toLowerCase()]) return interaction.reply(`Thats not a car!`)

            if(!usercars.includes(carname.toLowerCase())) return interaction.reply(`You don't have that car!`)
            ...
            let carhand = db.fetch(`${carindb.Name}handling_${user.id}`) || carindb.Handling
            ...
            let carlivery = db.fetch(`${carindb.Name}livery_${user.id}`) || carindb.Image

            db.push(`usedcarshop_${user.id}.Current`, {Car: carname, Price: carprice, Speed: carspeed, Acc: caracc, Handling: carhand, Exhaust: carexhaust, Intake: carintake, Tires: cartires, Turbo: carturbo, Bodykit: carbody, Spoiler: carspoiler, Gearbox: cargearbox, ECU: carecu, Clutch: carclutch, Livery: carlivery, Suspension: carsusp})
            ...
            let shopid = interaction.options.getUser("user").id || user.id
                ...
                for(i in usedlisted){
                    ...
                    used.push(`${cardb.Cars[uc.Car.toLowerCase()].Emote} ${uc.Car} : $${numberWithCommas(uc.Price)}`)
