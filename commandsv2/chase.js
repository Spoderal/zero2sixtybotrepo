...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if(bounty == 0) return interaction.reply("Negative, that user is innocent")
    let selectedpolice = interaction.options.getString("car");
    if(!selectedpolice[0]) return interaction.reply("Thats not a police car! Try using the ID of the car you want to use, or try using a police car")

    await interaction.reply(`${user}, ${policeuser} is chasing you! Use any items you can to lose them, or just run.\n\n${policeuser}, We have an APB on a racer, all units proceed and take him out.`);

    policecooldown.chasing = Date.now()
    policecooldown.save()
   selectedpolice = policedata.cars.filter((car) => car.ID == selectedpolice && car.police == true)
   selectedpolice = selectedpolice[0]

   let selectedracer = userdata.car_racing || lodash.sample(userdata.cars)

   let liveryracer = selectedracer.Livery || cardb.Cars[selectedracer.Name.toLowerCase()].Image

   let liverypolice = selectedpolice.Livery || cardb.Cars[selectedpolice.Name.toLowerCase()].Image
  ...
  if(policeitems.includes("spikes")){
    console.log("spikes")
    for (var i2 = 0; i2 < 1; i2++) policeitems.splice(policeitems.indexOf("spikes"), 1);
  ...
  let w = setInterval( async () => {
    ...
    if(timer >= 10){
      ...
      if(tracklength > tracklength2){
        ...
        embed.setDescription(`${user} your car has been impounded and you lost your bounty and ${toCurrency(bounty)}\n${policeuser} you gained ${bounty} bounty`)
    ...
    policedata.save()
       await msg.edit({embeds: [embed]})
        ...
        embed.setDescription(`${user} you gained ${toCurrency(bounty)}\n${policeuser} your police car has been impounded`)
