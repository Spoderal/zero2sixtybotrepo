...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    async execute(interaction) {
      ...
      let dailytask = db.fetch(`dailytask_${userid}`)
      let weeklytask = db.fetch(`weeklytask_${userid}`)
      ...
      if (timeoutweek - (Date.now() - timerforweekly) < 0) {  
        db.set(`weeklytask_${userid}`, {task: randomtask2.Task, user: userid, completed: false, reward: randomtask2.Reward})
        db.set(`weeklytasktimer_${userid}`, Date.now())
      } 
     
      if (timeout1 - (Date.now() - timerfordaily) < 0) {          
          db.set(`dailytask_${userid}`, {task: randomtask.Task, user: userid, completed: false, reward: randomtask.Reward})
      ...
      if(!dailytask1 || !weeklytask1) return interaction.reply("Please send a message to set your tasks.")
      let dailyemote
      let weeklyemote
     ...
     if(weeklytask1.completed == true) weeklyemote = "✅"
      ...
      .addField("Daily", `${dailytask1.task} : $${numberWithCommas(dailytask1.reward)} ${dailyemote} *${dailyms} left*`)
      .addField("Weekly", `${weeklytask1.task} : $${numberWithCommas(weeklytask1.reward)} ${weeklyemote} *${weeklyms} left*`)
