...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((cmd) => cmd
    .setName("list")
    .setDescription("List the tasks you can claim")

    )
    .addSubcommand((cmd) => cmd
    .setName("view")
    .setDescription("View your tasks and their progress")
  ...
  async execute(interaction) {
    ...
    if(subcommand == "list"){
      ...
      if(!taskindb) return interaction.reply("Thats not a task! Try using the ID of the task")
      if(filteredtask[0]) return interaction.reply("You already have this task in progress!")
      let filteredtime = usertasks.filter((task) => task.ID == `T${taskindb.ID}`)
      if(filteredtime[0]){
      ...
      for(let task in usertasks){
        ...
        if(t !== filteredtime[0] && t !== filteredtime2[0] && t !== filteredtime3[0]){
          tasksarr.push(`\`ID: ${t.ID}\`${t.Task} : ${toCurrency(t.Reward)} ${emote}`)

        }
      }
      if(tasksarr.length == 0) return interaction.reply("You don't have any tasks! Try claiming some with /tasks claim")
      let embed = new Discord.EmbedBuilder()
      .setTitle("Your tasks")
      .setDescription(`${tasksarr.join('\n')}`)
      .setColor(colors.blue)
