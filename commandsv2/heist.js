...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setName("user")
    .setDescription("The user to rob with")
    ),
  ...
  async execute(interaction) {
    ...
    if(user2){
      ...
      for(let it in itemsrequired){
        if(!userdata2.items.includes(itemsrequired[it])) return interaction.reply(`Your partner needs a ${itemsrequired[it]} to start this heist!`)
    }
    }
    ...
    let msg = await interaction.reply({embeds: [embed], fetchReply: true, components: [row1]})
    ...
    collector2.on("collect", async (i) => {
      ...
      if(user1ready == true && !user2){
        ...
        let randomCash = randomRange(locationindb.payoutrangelow, locationindb.payoutrangehigh)
        let randomChance = randomRange(1, 100)
    
         embed 
        .setTitle(`Starting heist for ${locationindb.emote} ${locationindb.name}`)
        .setThumbnail(locationindb.image)
        .setDescription(`This ${locationindb.name} has ${toCurrency(locationindb.payoutrangehigh)} stored\n\nDrilling...`)
        .setColor(colors.blue)
       ...
       await i.update({embeds: [embed], fetchReply: true})
        ...
        setTimeout(async () => {
          embed.setDescription(`This ${locationindb.name} has ${toCurrency(locationindb.payoutrangehigh)} stored\n\nOpening Vault...`)
        ...
        setTimeout(async () => {
          ...
          if(randomChance <= chance){
            embed.setDescription(`You stole ${toCurrency(randomCash)}! *If you have another user, you split it*\n\nYou gained ${emotes.bounty} 500 bounty`)
            await i.editReply({embeds: [embed], fetchReply: true})
            ...
            if(user2){
              ...
              for (var i2 = 0; i2 < 1; i2++) items2.splice(items2.indexOf("mask"), 1);
              userdata2.items = items2;

              for (var i = 0; i < 1; i++) items2.splice(items2.indexOf("crowbar"), 1);
              userdata2.items = items2;
              userdata2.save()
              ...
              userdata.bounty += 500
              for (var i4 = 0; i4 < 1; i4++) items.splice(items.indexOf("mask"), 1);
              userdata.items = items;

              for (var i3 = 0; i3 < 1; i3++) items.splice(items.indexOf("crowbar"), 1);
              userdata.items = items;
            }
            cooldowndata.save()
            userdata.save()
            ...
            if(user2){
              ...
              cooldowndatau2.heist = Date.now()
              cooldowndatau2.save()
              for (var i7 = 0; i7 < 1; i7++) items2.splice(items2.indexOf("mask"), 1);
              userdata2.items = items2;

              for (var i8 = 0; i8 < 1; i8++) items2.splice(items2.indexOf("crowbar"), 1);
              userdata2.items = items2;
              userdata2.save()
            }
            else {
              ...
              cooldowndata.heist = Date.now()
              userdata.bounty += 1000
              for (var i5 = 0; i5 < 1; i5++) items.splice(items.indexOf("mask"), 1);
              userdata.items = items;

              for (var i6 = 0; i6 < 1; i6++) items.splice(items.indexOf("crowbar"), 1);
              userdata.items = items;
            }
            cooldowndata.save()
            userdata.save()
