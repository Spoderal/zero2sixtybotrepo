...
module.exports = {
  ...
  async execute(interaction) {
    ...
    let exhaustpower = Math.round(
      parttiersdb[`exhaust${exhausttier}`].Power * selected[0].Speed
    );
    let turbopower = Math.round(
      parttiersdb[`turbo${turbotier}`].Power * selected[0].Speed
    );
    let tirepower = Math.round(
      parttiersdb[`tires${tiretier}`].Handling * selected[0].Handling
    );
    let suspensionpower = Math.round(
      parttiersdb[`suspension${suspensiontier}`].Handling * selected[0].Handling
    );
    let enginepower = Math.round(
      parttiersdb[`engine${enginetier}`].Power * selected[0].Speed
    );
    let clutchpower = Math.round(
      parttiersdb[`clutch${clutchtier}`].Power * selected[0].Speed
    );
    let intercoolerpower = Math.round(
      parttiersdb[`intercooler${intercoolertier}`].Power * selected[0].Speed
    );
    let intakepower = Math.round(
      parttiersdb[`intake${intaketier}`].Power * selected[0].Speed
    );
    let ecupower = Math.round(
      parttiersdb[`ecu${ecutier}`].Power * selected[0].Speed
    );
    let gearboxpower = Math.round(
      parttiersdb[`gearbox${gearboxtier}`].Handling * selected[0].Handling
    );
    ...
    let embed = new EmbedBuilder()
      ...
      .addFields(
        {
          ...
          value: `Tier: ${exhausttier}\nPower: ${exhaustpower}\nAcceleration: ${
            parttiersdb[`exhaust${exhausttier}`].Acceleration
          }`,
          inline: true,
        },
        {
        ...
        },
        {
          ...
          value: `Tier: ${turbotier}\nPower: ${turbopower}\nAcceleration: ${
            parttiersdb[`turbo${turbotier}`].Acceleration
          }`,
          inline: true,
        },
        {
          ...
          value: `Tier: ${intaketier}\nPower: ${intakepower}\nAcceleration: ${
            parttiersdb[`intake${intaketier}`].Acceleration
          }`,
          inline: true,
        },
        {
        ...
        },
        {
          ...
          value: `Tier: ${intercoolertier}\nPower: ${intercoolerpower}\nAcceleration: ${
            parttiersdb[`intercooler${intercoolertier}`].Acceleration
          }`,
          inline: true,
        },
        {
      ...
      .setDescription(
        `You will need to upgrade your tier 1 twice to get to the next tier.`
      );
