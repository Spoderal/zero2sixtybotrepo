           ...
           let timeout = 45000
            ...
            if (racing !== null && timeout - (Date.now() - racing) > 0) {
              ...
              if (racing3 !== null && timeout - (Date.now() - racing3) > 0) {
                ...
                return interaction.reply(`The 2nd user needs to wait ${time} before racing again.`)
              } 
            ...
            interaction.reply(`${user2}, what car do you wish to rally race ${user} in?`)
            ...
            collector.on('collect', msg => {
              ...
              if(!user2carchoice) return msg.reply("Specify an id! Use /ids select [id] [car] to select a car! \`Command example: /race @test 1\`")
              ...
              if(!selected2) return msg.reply("That id doesn't have a car! Use /ids select [id] [car] to select it! \`Command example: /race @test 1\`");
                ...
                collector.on('end', async () => {
                    interaction.channel.send(`${user3}, what car do you wish to rally race ${user} & ${user2} in?`)
                ...
                collector2.on('collect', msg2 => {
                    ...
                    if(!user3carchoice) return msg2.reply("Specify an id! Use /ids select [id] [car] to select a car! \`Command example: /race @test 1\`")
                     selected3 = db.fetch(`selected_${user3carchoice}_${msg2.author.id}`)
                    if(!selected3) return msg2.reply("That id doesn't have a car! Use /ids select [id] [car] to select it! \`Command example: /race @test 1\`");
                      let restoration2 = db.fetch(`${cars.Cars[selected3.toLowerCase()].Name}restoration_${msg2.author.id}`)
               ...
               let user3carzerosixty = db.fetch(`${cars.Cars[user3car.toLowerCase()].Name}060_${user3.id}`)
              ...
              if (user3carzerosixty < 2) {
                db.set(`${cars.Cars[user3car.toLowerCase()].Name}060_${user3.id}`, 2);
              }
              ...
              let newzero3 = db.fetch(`${cars.Cars[user3car.toLowerCase()].Name}060_${user3.id}`)

              let offroad = db.fetch(`${cars.Cars[user1car.toLowerCase()].Name}offroad_${user.id}`)
              let offroad2 = db.fetch(`${cars.Cars[user2car.toLowerCase()].Name}offroad_${user2.id}`)
              let offroad3 = db.fetch(`${cars.Cars[user3car.toLowerCase()].Name}offroad_${user3.id}`)

              if (!offroad || offroad == 0 || offroad < 0) {
                return interaction.channel.send(`${user}'s car's offroad rating is too low!`);
              }
              if (!offroad2 || offroad2 == 0 || offroad2 < 0) {
                return interaction.channel.send(`${user2}'s car's offroad rating is too low!!`);
              }
              if (!offroad3 || offroad3 == 0 || offroad3 < 0) {
                return interaction.channel.send(`${user3}'s car's offroad rating is too low!!`);
              }
              ...
              if(!db.fetch(`${cars.Cars[user3car.toLowerCase()].Name}handling_${user3.id}`)) {
                db.set(`${cars.Cars[user3car.toLowerCase()].Name}handling_${user3.id}`, cars.Cars[user3car.toLowerCase()].Handling)
              }
              ...
              let handling3 = db.fetch(`${cars.Cars[user3car.toLowerCase()].Name}handling_${user3.id}`) || cars.Cars[user3car.toLowerCase()].Handling
              ...
              let otherzero2sixty2 = db.fetch(`${cars.Cars[user3car.toLowerCase()].Name}060_${user3.id}`)
              ...
              let hp3 = user3carspeed / otherzero2sixty2 + handling12
               ...
               let range = db.fetch(`${cars.Cars[user1car.toLowerCase()].Name}range_${interaction.user.id}`)
               if(cars.Cars[user1car.toLowerCase()].Electric){
                 if(range <= 0){
                   return interaction.reply(`${user}, Your EV is out of range! Run /charge to charge it!`)
                 }
               }
               let range2 = db.fetch(`${cars.Cars[user2car.toLowerCase()].Name}range_${user2.id}`)
               if(cars.Cars[user2car.toLowerCase()].Electric){
                 if(range2 <= 0){
                  return interaction.reply(`${user2}, Your EV is out of range! Run /charge to charge it!`)
                }
               }
               let range3 = db.fetch(`${cars.Cars[user3car.toLowerCase()].Name}range_${user3.id}`)
               if(cars.Cars[user3car.toLowerCase()].Electric){
                 if(range3 <= 0){
                  return interaction.reply(`${user3}, Your EV is out of range! Run /charge to charge it!`)
                }
               }

               if(range){
                db.subtract(`${cars.Cars[user1car.toLowerCase()].Name}range_${user.id}`, 1)
  
            }
            if(range2){
              db.subtract(`${cars.Cars[user2car.toLowerCase()].Name}range_${user2.id}`, 1)

          }
          if(range3){
            db.subtract(`${cars.Cars[user3car.toLowerCase()].Name}range_${user3.id}`, 1)

        }
               let embed = new discord.MessageEmbed()
               .setTitle("3...2...1....GO!")
               .addField(`${user.username}'s ${cars.Cars[user1car.toLowerCase()].Emote} ${cars.Cars[user1car.toLowerCase()].Name}`, `Speed: ${user1carspeed}MPH\n\n0-60: ${newzero1}s\n\nOffroad Rating:${offroad}`)
               .addField(`${user2.username}'s ${cars.Cars[user2car.toLowerCase()].Emote} ${cars.Cars[user2car.toLowerCase()].Name}`, `Speed: ${user2carspeed}MPH\n\n0-60: ${newzero2}s\n\nOffroad Rating:${offroad2}`)
               .addField(`${user3.username}'s ${cars.Cars[user3car.toLowerCase()].Emote} ${cars.Cars[user3car.toLowerCase()].Name}`, `Speed: ${user3carspeed}MPH\n\n0-60: ${newzero3}s\n\nOffroad Rating:${offroad3}`)
                 ...
                 if(timer >= 10){
                   ...
                   if(tracklength > tracklength2 && tracklength > tracklength3){
                       ...
                       if(!db.fetch(`2ezbadge_${user.id}`)){
                        ...
                        interaction.channel.send(`${user}, you just earned the "2EZ" badge for winning a group race!`)
                      }
                    
                       return;
                   }
              
                   else if(tracklength2 > tracklength && tracklength2 > tracklength3){
                      console.log("End")
                      clearInterval(x)
                      ...
                      if(!db.fetch(`2ezbadge_${user2.id}`)){
                        ...
                        interaction.channel.send(`${user2}, you just earned the "2EZ" badge for winning a group race!`)
                      }
                 
                      return;
               
                   }
                   else if(tracklength3 > tracklength2 && tracklength3 > tracklength){
                      console.log("End")
                      clearInterval(x)
                      ...
                      if(!db.fetch(`2ezbadge_${user3.id}`)){
                        ...
                        interaction.channel.send(`${user3}, you just earned the "2EZ" badge for winning a group race!`)
                      }
