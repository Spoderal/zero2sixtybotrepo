...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if(!partdb.Parts[partoption.toLowerCase()]) return interaction.reply(`Thats not a part!`)

    let fusable = partdb.Parts[partoption.toLowerCase()].Fuse

    if(!fusable) return interaction.reply("Thats not a installable part! If its a legacy part, wait for it to be implemented.")

    if(!selected[0]) return interaction.reply(`Thats not a car! Did you make sure to put the cars ID?`)
    ...
    await User.findOneAndUpdate(
      ...
      interaction.reply(`Installed a ${partindb.Emote} ${partindb.Name} on your ${selected[0].Name}`)
  },
};
