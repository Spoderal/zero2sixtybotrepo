...
const cardb = require("../data/newcars.json");
  ...
  async execute(interaction) {
    ...
    if (!selected)
      return interaction.reply({
        content: `The id, "${idtoselect}" doesn't have a car! Use /ids select [id of your choice] [car] to select a car to it!`,
        ephemeral: true,
      });
    ...
    if (!carstats) {
      ...
      interaction.reply({
        content: `There was an error loading the stats for your car, so its been set back to default. Please join the support server and open a ticket if this was a mistake.`,
        ephemeral: true,
      });
      return;
    }
