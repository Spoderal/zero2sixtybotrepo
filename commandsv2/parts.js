...
module.exports = {
  ...
  async execute(interaction) {
    ...
    let embed = new EmbedBuilder()
      ...
      let msg = await interaction.reply({content: "Please wait...", fetchReply: true})
      let num = 0
      ...
      for(let p in parts.Parts){
        ...
        if(part.Name && !part.Name.startsWith("J") && !part.Name.startsWith("BM") && part.Price > 0){
      ...
      let rowbuy = new ActionRowBuilder().addComponents(
       ...
       await interaction.editReply({ embeds: [embed], components: [row, prow2], fetchReply: true })
        ...
        if(i.customId.includes("exhaust")){
          ...
          if(part.DecreaseSpeed > 0){
           ...
           if(part.AddedSixty > 0){
            statsdisp.push(`${emotes.zero2sixty} Acceleration: -${part.AddedSixty}`)
          }
          if(part.DecreasedSixty > 0){
            statsdisp.push(`${emotes.zero2sixty} Acceleration: +${part.DecreasedSixty}`)
          }
          if(part.AddHandling > 0){
            statsdisp.push(`${emotes.handling} Handling: +${part.AddHandling}`)
          }
          if(part.DecreasedHandling > 0){
            statsdisp.push(`${emotes.handling} Handling: -${part.DecreasedHandling}`)
          }
          if(part.AddWeight > 0){
          ...
          i.update({embeds: [embed2], fetchReply: true, components: [row, prow2, row9, rowbuy, rowbuy]})
          ...
          i.update({embeds: [embed2], fetchReply: true, components: [row, prow2, row9, rowbuy]})
          ...
          if(userdata.cash < part.Price || part.Price <= 0) return i.update({content:"You can't afford this part!", embeds: [], components: []}) 
