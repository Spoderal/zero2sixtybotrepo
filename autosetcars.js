...
for(c in carsall){
    ...
    try{
        ...
        let handling = db.fetch(`${cardb.Cars[car.toLowerCase()].Name}handling_${id}`) || carindb.Handling
        ...
        let suspension = db.fetch(`${cardb.Cars[car].Name}suspension_${id}`) || null
        
        let weight = db.fetch(`${cardb.Cars[car].Name}weight_${id}`) || null
        let offroad = db.fetch(`${cardb.Cars[car].Name}offroad_${id}`) || null
        let drift = db.fetch(`${cardb.Cars[car].Name}drift_${id}`) || carindb.Drift || 0
        let speed = db.fetch(`${cardb.Cars[car].Name}speed_${id}`) || carindb.Speed
        let zerosixty = db.fetch(`${cardb.Cars[car].Name}060_${id}`) || carindb['0-60']
        
        let nitro = db.fetch(`${cardb.Cars[car].Name}nitro_${id}`) || null
        let restoration  = db.fetch(`${cardb.Cars[car].Name}restoration_${id}`) || null
        let resale  = db.fetch(`${cardb.Cars[car].Name}resale_${id}`) || cardb.Cars[car].Price * 0.65 || 0
        let range  = db.fetch(`${cardb.Cars[car].Name}range_${id}`) 
        
        let engine = db.fetch(`${cardb.Cars[car].Name}engine_${id}`)
        
        let carimage = db.fetch(`${cardb.Cars[car].Name}livery_${id}`) || cardb.Cars[car].Image
