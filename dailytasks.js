...
module.exports = (client) => {
    client.on('messageCreate', async (message) => {
        ...
        if (timeoutweek - (Date.now() - timerforweekly) > 0) {  
            ...
            db.set(`weeklytask_${member.id}`, {task: randomtask2.Task, user: member.id, completed: false, reward: randomtask2.Reward})
        ...
        if (timeout - (Date.now() - timerfordaily) > 0) {          
            return;
              ...
              db.set(`dailytask_${member.id}`, {task: randomtask.Task, user: member.id, completed: false, reward: randomtask.Reward})
