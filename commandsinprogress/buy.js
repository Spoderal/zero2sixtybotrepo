...
module.exports = {
  ...
  async execute(interaction) {
    ...
    for(let house in houses){
      ...
      let car = cars.find(c => c.alias.toLowerCase() === bought || c.Name.toLowerCase() === bought)
      ...
      if(item && bought !== "pvp crate"){
        ...
        if(car.Price == 0 && !car.Heart && !car.Carver && !car.Tokens) return interaction.reply("This car isn't purchasable!")
        ...
        if(car.Carver){
          if(userdata.carver < 500){
            return await interaction.reply({ content: `You need 500 carver cash to buy this car`, ephemeral: true })
          }
          userdata.carver -= 500
          let carobj = {
            ID: car.alias,
        ...
        if(car.Valentines){
          if(userdata.hearts < car.Heart){
            return await interaction.reply({ content: `You need ${car.Heart} hearts to buy this car`, ephemeral: true })
          }
          userdata.hearts -= car.Heart
          let carobj = {
            ID: car.alias,
            Name: car.Name,
            Speed: car.Speed,
        ...
        .setDescription(`You bought a ${car.Emote} ${car.Name} for ${emotes.cash} ${toCurrency(car.Price)}`)
        .setColor(colors.blue)
